function results = run_COOT(seq, res_path, bSaveImage)
% Entry point for the Wu - CVPR2013 benchmark
    ...
    pos = floor([seq.init_rect(1,2), seq.init_rect(1,1)]+target_sz/2);
     ...
     addpath('D:\benchmark\VOT\tracker\examples\matlab\SiamCF\external_libs\matconvnet\matlab\mex\');
     addpath('D:\benchmark\VOT\tracker\examples\matlab\SiamCF\external_libs\matconvnet\matlab\');
    grayscale_sequence = false;	% suppose that sequence is colour
    hog_cell_size = 4;
           ...
           if frame == 10
                ...
                for i = 1:size(response,1)
                    for j = 1:size(response,2)
                        entropy = entropy + (abs(i-row)*abs(j-col))*response(i,j)/sum_response;
                    end
                end
                ada_entropy = entropy;
                threshold=ada_entropy*1.20;
            end
            ...
            if frame > 10
                ...
                ada_entropy = (1-thresh_lambda)*entropy+thresh_lambda*ada_entropy;
                threshold=ada_entropy*1.20;
            end
            ...
            if(entropy<threshold)
                ...
                %mu = learning_rate_gauss* mu + (1- learning_rate_gauss)*max(response(:));
                %sigma = (learning_rate_gauss*sigma^2+learning_rate_gauss*(1-learning_rate_gauss)*(mu-max(response(:)))^2)^(0.5);
                ...
                x_crops = make_scale_pyramid(img, pos, scaledInstance, p.instanceSize, avgChans, stats, p);
                % evaluate the offline-trained network for exemplar x features
                [newTargetPosition, newScale] = tracker_eval(net_x, round(s_x), scoreId, z_features, x_crops, pos, window, p);
                scale_factor = (1-p.scaleLR)*scale_factor+p.scaleLR * scale_factor*scales(newScale);
